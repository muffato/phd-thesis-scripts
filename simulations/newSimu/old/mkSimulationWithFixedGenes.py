...
def buildGenomes(genome, node):
	...
	if node in phylTree.items:
		...
		for (fils,dist) in phylTree.items[node]:
			...
			nbEvents = [int(round(arguments[x]*dist*randomAccel())) for x in ["chrInvertRate","chrTranslocRate","chrFusionRate","chrBreakRate"]] + geneEvolution[(node,fils)]
			...
			dupDetails = []
			nb = 0
			for (x,n) in nbEvents[5].iteritems():
				nb += (x-1)*n
				# dupDetails contient le nombre de genes a dupliques
				dupDetails.extend( [(0,x-1)] * n )
			nbEvents[5] = nb
			...
			for evt in events:
				...
				# Duplication de gene
					...
					# Choix d'une configuration de duplication
					target = random.randint(0, len(dupDetails)-1)
					(nexist,ntarget) = dupDetails[target]
					# On cherche un gene qui s'est duplique nexist fois (l'objectif est ntarget)
					if nexist == 0:
						# Aucune duplication pre-requise
						while True:
						...
						# Le gene est choisi parmi ceux qui se sont dupliques le bon nombre de fois
						i = random.choice([x for (x,l) in dup.iteritems() if len(l) == nexist])
					...
					nexist += 1
					if nexist == ntarget:
						del dupDetails[target]
					else:
						dupDetails[target] = (nexist,ntarget)
					nbTotalGenes += 1
...
arguments = utils.myTools.checkArgs( \
	[("phylTree.conf",file), ("genesStatsFile",str), ("root",str), ("iniGeneNumber",int), ("iniChrNumber",int)], \
...
geneEvolution = {}
f = utils.myTools.myOpenFile(arguments["genesStatsFile"], "r")
for l in f:
	t = l.replace('\n','').split('\t')
	geneEvolution[(t[0],t[1])] = [int(t[2]),eval(t[3]),int(t[5])]
f.close()
