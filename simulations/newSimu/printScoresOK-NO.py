...
def doSim(sim):
	...
	chromAssoc = {}
	for (c,diag,_) in utils.myTools.readTabular(arguments["IN.genomeFile"] % sim, [int,str,str]):
		for i in diag.split():
			chromAssoc[int(i)] = c

	# On charge les diagonales
	numDiagsOK = set()
	for (i,(_,_,diag,_,_,_)) in enumerate(utils.myTools.readTabular(arguments["IN.diagsFile"] % sim, [str,int,str,str,str,str])):
	...
	for (i1,i2,score) in utils.myTools.readTabular(arguments["IN.scoresFile"] % sim, [int,int,str], ' '):
