...
for anc in listAncestr:
	...
	dicGenomes[anc] = utils.myGenomes.Genome(arguments["IN.ancGenomeFromDiags"] % phylTree.fileName[anc], ancGenes=genesAnc[anc])
...
def getAllAdj(anc):
	...
	for esp in listSpecies:
		...
		for (n,((c1,d1),(c2,d2),da)) in enumerate(myDiags.calcDiags(dicGenomes[esp], dicGenomes[anc], genesAnc[phylTree.dicParents[anc][esp]], 0, True, orthosFilter)):
