	...
	for (v, c1, c2) in countPara(options["nbMinParaCouple"]):
		if len(rep[c1]) == 0 and len(rep[c2]) == 0:
			...
			for i in rep[x]:
				...
				if count3.get(tuple(t), 0) > options["coefMinAltern3"]*nbPara[i]:
...
# Arguments
(noms_fichiers, options) = myTools.checkArgs(["genes_list.conf", "GENES_ANC"], [("especeDupliquee", str, 'T'), ("bestScore", bool, False), ("precisionChrAnc", int, 1000000), ("nbGenesTransloc", int, 4), ("nbMinParaCouple", int, 4), ("coefMinAltern3", float, 1.), ("loopChrAnc", bool, False)] )
