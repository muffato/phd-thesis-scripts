...
NF==12 {
	...
	box( x1+cmX(minT1), y1+cmY(minH), cmX(maxT1-minT1), cmY(maxH-minH), color[chT1]);
	box( x1+cmX(minT2), y1+cmY(minH), cmX(maxT2-minT2), cmY(maxH-minH), color[chT2]);
}
...
$1 == "RECT" {
	...
	box( x1+cmX($2)-dp/2, y1+cmY($4)-dp/2, cmX($3-$2)+dp, cmY($5-$4)+dp, Cb, Cr)
}
...
$1 == "POINT" {
	if (NF == 4)
		...
		Cb = "black"
	box( x1+cmX($2), y1+cmY($3), dp, dp, Cb, Cb);
}
