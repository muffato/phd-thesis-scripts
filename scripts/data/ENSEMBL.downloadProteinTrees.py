...
###########################################
def extractGeneFamilies(node, anc, previousDup):
	...
	# Les intermediaires
	if newDup:
		toWrite = phylTree.dicLinks[newAnc][anc][1:-1]
	elif previousDup:
		toWrite = phylTree.dicLinks[newAnc][anc]
	else:
		toWrite = phylTree.dicLinks[newAnc][anc][:-1]
...
for (root,_) in data[1]:
	if 'taxon_name' in info[root]:
		...
		rebuildTree(root)
		extractGeneFamilies(root, info[root]['taxon_name'], False)
