...
def buildGenomes(genome, node, dupPairs):
	...
	if node in phylTree.items:
		...
		# Pour chaque fils
		for (fils,dist) in phylTree2.items[node]:
			fils = fils.replace(".", " ")
			...
			dist *= len(correspondances) * arguments["eventRate"] / (2*arguments["chrInvertRate"]+3*arguments["chrTranslocRate"]+arguments["chrFusionRate"]+arguments["chrBreakRate"])
			todelete = set()
			nbDup = 0
...
arguments = utils.myTools.checkArgs( \
	[("phylTree.conf",file), ("phylTree2.conf",file), ("genesStatsFile",str), ("iniChrNumber",int)], \
	# Parametres d'evolution
	[("eventAccel",float,3.), ("vonMisesMean",float,.2), ("vonMisesKappa",float,2.), ("eventRate",float,1.0), \
...
phylTree2 = utils.myPhylTree.PhylogeneticTree(arguments["phylTree2.conf"])
