...
for (anc,randomWalks,genes,weightM,weightP) in utils.myTools.myIterator.tupleOnManyLists(phylTree.listAncestr,[2,3,4,5,7,10,15,25],"-+","-+","-+"):
	ancF = phylTree.fileName[anc]
	print "Output = /workspace/muffato/tmp/allsimu/%d/communities/%s.%d.%s.%s.%s.res" % (indSim,ancF,randomWalks,genes,weightM,weightP)
	print "Error = /workspace/muffato/tmp/allsimu/%d/communities/%s.%d.%s.%s.%s.log" % (indSim,ancF,randomWalks,genes,weightM,weightP)
	print "Arguments = +psyco /users/ldog/muffato/work/phylTree.vertebrates.45.conf /workspace/muffato/tmp/allsimu/%d/res.diags.bz2 -ancestr=%s +useOutgroups %suseLonelyGenes %sweightNbChr- %sweightNbChr+ -scoringMethod=2 -walktrapLength=%d -genesFile=/workspace/muffato/tmp/allsimu/%d/genes/genes.%%s.list.bz2 -ancGenesFile=/workspace/muffato/tmp/allsimu/%d/ancGenes/ancGenes.%%s.list.bz2" % (indSim,anc.replace(' ','^'),genes,weightM,weightP,randomWalks,indSim,indSim)
	print "Log = /users/ldog/muffato/condor/communities.%d.%s.%d.%s.%s.%s.condor_log" % (indSim,ancF,randomWalks,genes,weightM,weightP)
	print "Queue"
	print
