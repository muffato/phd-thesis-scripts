...
if (options["output"] == modeMatrix):
	...
	utils.myCairoOutput.printPsHeader(0.001)
	if len(options["backgroundColor"]) > 0:
		utils.myCairoOutput.drawBox(0,0, 21,29.7, options["backgroundColor"], options["backgroundColor"])
	sys.stderr.write('.')
	...
	lstNum1 = prepareGenome(table12, lambda x: utils.myCairoOutput.drawLine(1 + x*scaleX, 1, 0, float(sum([len(table21[c]) for c in table21]))*scaleY, options["penColor"]))
	sys.stderr.write('.')
	lstNum2 = prepareGenome(table21, lambda y: utils.myCairoOutput.drawLine(1, 1 + y*scaleY, 19, 0, options["penColor"]))
	sys.stderr.write('.')
	...
	for c1 in table12:
		for i1 in table12[c1]:
			...
			for (c2,i2) in table12[c1][i1]:
				...
				coul = utils.myCairoOutput.getColor(coul, options["defaultColor"])
				utils.myCairoOutput.drawBox( xx, yy, dp, dp, coul, coul)

	utils.myCairoOutput.printPsFooter()
	print >> sys.stderr, " OK"
...
# Caryotype
	...
	utils.myCairoOutput.printPsHeader()
	...
	for c in chr1:
		...
		utils.myCairoOutput.drawText(xx, y0, str(c), options["penColor"])
		y = y0 + 1
		...
		for i in table12[c]:
			...
			if len(tmp) == 0:
				...
				col = utils.myCairoOutput.getColor(str(tmp[0][0]), options["defaultColor"])
			if col == last:
				nb += 1
			else:
				if nb != 0:
					utils.myCairoOutput.drawBox(xx, y, dx, nb/dy, last, last)
					y += nb/dy
				last = col
				nb = 1
		if nb != 0:
