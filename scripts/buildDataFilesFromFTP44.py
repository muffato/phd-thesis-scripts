...
def proceedFile(fin, fout, foutR):
	...
	try:
		...
		while True:
			...
			print >> fout, "\t".join([c[7],c[4],c[24], c[i2-36],c[i2-39],c[i2-19], newAnc, c[45],c[46], c[i2+4],c[i2+5], c[0]])
			if foutR != None:
				print >> foutR, "\t".join([c[i2-36],c[i2-39],c[i2-19], c[7],c[4],c[24], newAnc, c[i2+4],c[i2+5], c[45],c[46], c[0]])
			if newAnc == theoryAnc:
				nb2 += 1
			c = fin.next().split('\t')
