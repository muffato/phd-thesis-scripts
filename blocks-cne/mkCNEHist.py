...
# Chargement des CNE
CNE = {}
for line in utils.myTools.readTabular(arguments["KX_HDMC_blocs.list"], [str,int,int,int,str]):
	# Les infos generales du CNE
	CNE[line[0]] = (line[1:],[])


for line in utils.myTools.readTabular(arguments["KX_HDMC_especes.list"], [str,str,int,int,str,str,int,str]):
...
for t in utils.myTools.readTabular(sys.stdin, [str]*10):
	if "sapiens" in t[2]:
		do = t[4]
	elif "sapiens" in t[5]:
		do = t[7]
